...
function selectProvince(type, con, isremove) {
    ...
    if (type == "all") {//显示省级
        ...
        function showProvince(list){
            ...
            for (var i=0;i<list.length;i++) {
                strVarCity += '<li><a href=\"javascript:;\" data-id=\"'+ list[i].PID +'\" data-code=\"' + list[i].TSA_CODE + '\" data-name=\"' + list[i].TSA_NAME + '\" class=\"d-item\"  onclick="selectProvince(\'sub\',this,\'\')">' + list[i].TSA_NAME + '<label>0</label></a></li>';
            }
            strVarCity += "                                <\/ul>";
            strVarCity += "                              <\/div>";
    ...
    //显示下一级   //当type为sub的情况下
        ...
        var codeLen = $(con).attr("data-code").length;
        var codeId = $(con).attr("data-id");
        //省或者市一级，添加下一级
        if ( codeLen == 2 || codeLen == 4) {
            ...
            function showCityOrQu(list){
                //添加标题
                if (isremove != "remove") {
                    $('.data-tabs ul').append('<li data-code=' + $(con).attr("data-code") + ' data-name=' + $(con).attr("data-name") + ' class="active" onclick="removenode_area(this)"><a href="javascript:;"><span>' + $(con).attr("data-name") + '</span><em></em></a></li>');
                }
                //添加内容
                strVarCity += "<ul class=\"clearfix\">";
                strVarCity += '<li class="" style="width:100%;font-size:16px;"><a href="javascript:;" class="d-item data-all"  data-code="' + $(con).attr("data-code") + '"  data-name=\"' + $(con).attr("data-name") + '\"  onclick="selectitem_area(this)">' + $(con).attr("data-name") + '<label>0</label></a></li>';
                for (var i = 0;i<list.length;i++) {
                    strVarCity += '<li><a href="javascript:;" class="d-item" data-id="'+ list[i].CID +'" data-code="' + list[i].TSA_CODE + '"  data-name=\"' + list[i].TSA_NAME + '\" onclick="selectProvince(\'sub\',this,\'\')">' + list[i].TSA_NAME + '<label>0</label></a></li>';
                }
                strVarCity += "<\/ul>";
                $('.data-container-city').html(strVarCity);
            }
...
function selectitem_area(con) { //选中区域
