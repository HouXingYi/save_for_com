...
;(function($){
    ...
    Hpage.prototype = {
        ...
        refresh : function(opts){
            ...
            this.numForEachPage = this.opts.numForEachPage;
            that.initBoxHtml();
            that.changeIndex(that.opts.initPage,that.container.find('.sePageItem[index="'+ that.opts.initPage +'"]'),1);
        },
        bind :function(){
            var that = this;
            //一页几条
            this.container.on("keydown", ".lenInput" ,function(e){
                if(e.keyCode == 13){
                    ...
                    that.changeIndex(1,that.container.find('.sePageItem[index=1]'));
                }
            });
            //到第几页
        ...
        // 跳转到指定页面
        changeIndex : function(index,item,isFirst){
            var that = this;
